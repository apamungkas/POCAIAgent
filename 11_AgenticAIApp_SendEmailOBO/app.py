...
# Helpers to detect & prepare email payload --------------------------------
...
def try_extract_email_payload(text: str):
    ...
    # 2) Fallback: first raw JSON-looking object if present
    if not candidates:
        # Heuristic: grab the first {...} with "subject" and "bodyHtml" inside
...
class AzureAIFoundryApp:
    ...
    def display_email_send_panel(self):
        ...
        if submitted:
            ...
            # persist debug
            # CHANGED: only API_BASE used to build the endpoint
            ...
            if status == 200:
                ...
                # clear pending draft so the panel hides
                st.session_state.pop("pending_email", None)
            else:
    ...
    def display_main_chat_interface(self):
        ...
        with chat_container:
            for message in st.session_state["chat_history"]:
                with st.chat_message(message["role"]):
                    # render as markdown so links are clickable after rerun
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    try:
                        ...
                        # send role header so backend picks the right agent
                        ...
                        # capture request info for persistent debug
                        ...
                        # store response for persistent debug (JSON or raw text)
                        try:
                            resp_json = resp.json()
                        except Exception:
                            resp_json = resp.text
                        ...
                        if resp.status_code == 200:
                            ...
                            # Prefer answer_md (markdown) if present, otherwise fall back to answer
                            ...
                            # Render sources as URLs only, and persist them into history
                            ...
                            if urls_only:
                                ...
                                # ...and make it part of the message stored in history so it doesn't disappear on rerun
                                rendered = f"{rendered}\n\n{sources_md}"

                            if data.get("thread_id"):
                                st.session_state["thread_id"] = data["thread_id"]

                            # store the rendered content (markdown or text + sources) to history
                            st.session_state["chat_history"].append({"role": "assistant", "content": rendered})

                            # try to extract a draft email payload from the assistant's output
                            draft = try_extract_email_payload(ai_md or ai_text)
                            if draft:
        ...
        # Show the send-as-user panel if we have a draft ready
